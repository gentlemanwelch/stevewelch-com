...
#
# WHY THIS EXISTS: WordPress has no "export media library" button, and the WXR
# export records only URLs, not bytes. But every one of these files is still
# served publicly by the live site, so a plain download works — no plugin, no
# FTP, no hosting login. Run this from a machine that can reach stevewelch.com,
...
#
# FILENAME COLLISIONS: six files were uploaded twice, to different month folders
# under wp-content/uploads, with identical names. An earlier version of this
# script named every download after its URL's last path segment, so the second
# copy of each looked like a file already downloaded and was skipped — six
# originals silently never came down. They are now written with a YYYY-MM prefix
# so both survive. The earliest upload of each keeps the bare filename, because
# that is the one content/media-manifest.ts references.
#
# Usage:
#   bash scripts/download-media.sh
#
# Re-running is safe and resumes: existing files are skipped by name, and every
...
fetch "https://stevewelch.com/wp-content/uploads/2024/06/W19_9376.jpg" "2024-06-W19_9376.jpg"
fetch "https://stevewelch.com/wp-content/uploads/2024/07/DSC_0119-scaled.jpg" "2024-07-DSC_0119-scaled.jpg"
fetch "https://stevewelch.com/wp-content/uploads/2024/07/DSC_0503-scaled.jpg" "2024-07-DSC_0503-scaled.jpg"
fetch "https://stevewelch.com/wp-content/uploads/2024/07/Restore-36-scaled.jpg" "2024-07-Restore-36-scaled.jpg"
fetch "https://stevewelch.com/wp-content/uploads/2024/07/Restore-36-scaled_1600x1200_acf_cropped-1.jpg" "2024-07-Restore-36-scaled_1600x1200_acf_cropped-1.jpg"
fetch "https://stevewelch.com/wp-content/uploads/2024/07/Restore-36-scaled_1600x1200_acf_cropped-2.jpg" "2024-07-Restore-36-scaled_1600x1200_acf_cropped-2.jpg"
...
# Every entry above has a distinct destination name, so the file count is an
# exact check rather than an approximation. A shortfall means a real miss.
present=$(find "$DEST" -type f ! -name '.*' | wc -l | tr -d ' ')
echo "  files on disk: $present of 155 expected"
if [ "$present" -lt 155 ]; then
  echo
  echo "  ⚠ Fewer files than expected. Re-run, or report the failures below."
fi
...
if [ "$fail" -gt 0 ]; then
  ...
  printf "$failed_list\n"
  echo
  echo "Tell Claude which ones failed — the site degrades gracefully when an"
  echo "image is missing, so nothing breaks."
fi
echo
